...
b_choice_L <- box(title = "Latent Dirichlet Allocation (LDA)", status = "success", 
                  ...
                  fluidRow(column(width = 12,
                                 introBox(bsButton("beta", label = "BETA", icon = icon("spinner"), style = "success"),
                                          bsButton("gamma",label = "GAMMA", icon = icon("spinner"), style = "success")
                                          )
                                )
                        )
) 
...
b_simulation <- box(title = "Inputs", status = "warning",
    ...
    selectInput("gender", "Gender:", c("Female" = "female", "Male" = "male")),
    sliderInput("age", "Age: ", 17, 75, 25),
    textInput("fav", "Favorite kind of shoes"),
    actionButton("action", label = "Enter")
)

b_slider <- box(title = "Training Size", status = "success",  width = 6, height = 150, solidHeader = TRUE, 
                sliderInput("slide",  "Size in %:", min = 30, max = 90, step= 1, value = 35))

b_accuracy <- infoBoxOutput("accu", width = 4)

b_words <- tableOutput("words")

b_prediction <- infoBoxOutput("prediction", width = 6)
...
#Panel Simulation
sim_row1 <- fluidRow(b_simulation)
sim_row2 <- fluidRow(b_prediction)
...
body <- dashboardBody(
    tabItems(tabItem(tabName = "Exec", intro_row1, intro_row2, intro_row3, intro_row4),
             ...
             tabItem(tabName = "Simulation", sim_row1, sim_row2),
             tabItem(tabName = "Business", in_row1, in_row2, in_row3, in_row4),
             tabItem(tabName = "Video", tags$video(src = "Team7.mp4", type = "video/mp4", autoplay = NA, controls = "controls"))
    )
)
##########################################################
